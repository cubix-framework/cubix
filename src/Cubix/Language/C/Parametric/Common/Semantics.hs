...
import Data.Comp.Multi ( AnnTerm, Context, Cxt(..), (:<:), project, appCxt,  inject' )
...
instance {-# OVERLAPPING #-} (CBinaryOp :<: g) => GetStrictness' g CExpression where
...
instance {-# OVERLAPPING #-} InsertAt' CStatement MCSig BlockItemL where
  ...
  insertAt' EnterEvalPoint t s =  liftM convertTerm $ liftM appCxt $ annotateM e
    where
      e :: Context MCSig (AnnTerm _ MCSig) CStatementL
      e = iCLabeledBlock NilF' (iBlock (insertF [Hole t, injF ((Hole $ fromJust $ dynProj $ inject' s) :: Context _ _ CStatementL)]) EmptyBlockEnd')
