...
import Data.Comp.Multi ( (:<:), Term, HTraversable, HFunctor )
...
type CanHoist f = ( VariableInsertionVariation f NoConstraint NoConstraint
                  ...
                  , DynCase (Term f) BlockL
                  , DynCase (Term f) IdentL
                  , DynCase (Term f) SingleLocalVarDeclL
                  , DynCase (Term f) [(StatSort f)]
                  , DynCase (Term f) [BlockItemL]
                  , Typeable (StatSort f)
                  )

type HoistMonad f = WriterT [Term f BlockItemL] (State (HoistState f))
type MonadHoist f m = (MonadWriter [Term f BlockItemL] m, MonadState (HoistState f) m)
...
declToAssign :: (CanHoist f) => Term f MultiLocalVarDeclCommonAttrsL -> Term f SingleLocalVarDeclL -> [Term f (StatSort f)]
...
refCheck :: (CanHoist f) => Set String -> Term f SingleLocalVarDeclL -> Bool
...
refCheckMulti :: (CanHoist f) => Set String -> [Term f SingleLocalVarDeclL] -> Bool
...
referencedIdents :: (CanHoist f) => Term f l -> Set String
...
cumulativeReferencedIdents :: forall f l. (CanHoist f) => [Term f l] -> [Set String]
cumulativeReferencedIdents = init . scanl addIdents Set.empty
  where
    addIdents :: Set String -> Term f l -> Set String
...
transformSingle :: (CanHoist f, SingleDecInsertion f NoConstraint, MonadHoist f m) => Term f (StatSort f) -> MaybeT m [Term f (StatSort f)]
...
transformMulti :: (CanHoist f, MultiDecInsertion f NoConstraint, MonadHoist f m) => Term f (StatSort f) -> MaybeT m [Term f (StatSort f)]
...
transformListHead :: (MonadPlus m, Typeable l, HFunctor f, ListF :<: f) => (Term f l -> m [Term f l]) -> RewriteM m (Term f) [l]
transformListHead f (ConsF' t ts) = f t >>= return . (foldr (ConsF' . injF) ts)
transformListHead _ NilF' = mzero

transformStatSorts :: forall f m. (CanHoist f, MonadHoist f m) => RewriteM (MaybeT m) (Term f) [(StatSort f)]
...
transformBlockItems :: forall f m. (CanHoist f, MonadHoist f m) => RewriteM (MaybeT m) (Term f) [BlockItemL]
transformBlockItems t = mapF injF <$> (transformStatSorts $ mapF fromProjF t)

addIdents :: (CanHoist f, MonadHoist f m) => RewriteM (MaybeT m) (Term f) IdentL
addIdents (Ident' s) = (seenIdents %= Set.insert s) *> mzero

updateState :: (CanHoist f, MonadHoist f m) => GRewriteM (MaybeT m) (Term f)
updateState = promoteRF addIdents >+> (\t -> updateSpecialState t *> mzero)

tillFailure :: (Monad m, HTraversable f) => GRewriteM (MaybeT m) (Term f) -> GRewriteM m (Term f)
...
transformOuterScope :: (MonadHoist f m, CanHoist f) => GRewriteM m (Term f) -> GRewriteM (MaybeT m) (Term f) -> GRewriteM m (Term f)
...
hoistBlock :: forall f. (CanHoist f) => Rewrite (Term f) BlockL
hoistBlock (Block' items end) = return $ Block' (insertF $ decls ++ extractF items') end
  where
    hoist :: HoistMonad f (Term f [BlockItemL])
...
hoistDeclarations :: (CanHoist f) => GRewrite (Term f)
